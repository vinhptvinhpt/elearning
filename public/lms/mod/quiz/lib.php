...
function quiz_add_instance($quiz) {
    ...
    $quiz_end = "quiz_end";
    $quiz_status = 0;
    $oldquiz = $DB->get_record('quiz', array('id' => $quiz->id));
    $course_id = $oldquiz->course;
    $action = "create";
    if ($quiz->timeopen){
        $noti = new stdClass();
        $noti->type = $quiz_type;
        $noti->target = $quiz_start;
        $noti->status_send = $quiz_status;
        $noti->course_id = $course_id;
        $noti->date_quiz = $quiz->timeopen;
        $noti->action = $action;
        $noti->quiz_id = $quiz->id;
        $quiz_info = new stdClass();
        $quiz_info->quiz_id = $quiz->id;
        $quiz_info->quiz_name = $quiz->name;
        $content = json_encode($noti);
        $info = json_encode($quiz_info);
        $noti_quiz = 'INSERT INTO tms_nofitications (type,target,status_send,course_id,date_quiz,content) values ("' . $noti->type . '","' . $noti->target . '", ' . $noti->status_send . ',' . $noti->course_id . ', '.$noti->date_quiz .',\''.$info.'\')';
        $DB->execute($noti_quiz);
        $noti_quiz_log = 'INSERT INTO tms_nofitication_logs (type,target,action,content,status_send,course_id) values ("' . $noti->type . '","' . $noti->target . '","' . $action . '",\'' . $content . '\', ' . $noti->status_send . ',' . $noti->course_id . ')';
        $DB->execute($noti_quiz_log);
    }
    if($quiz->timeclose){
        $noti = new stdClass();
        $noti->type = $quiz_type;
        $noti->target = $quiz_end;
        $noti->status_send = $quiz_status;
        $noti->course_id = $course_id;
        $noti->date_quiz = $quiz->timeclose;
        $noti->action = $action;
        $noti->quiz_id = $quiz->id;
        $content = json_encode($noti);
        $quiz_info = new stdClass();
