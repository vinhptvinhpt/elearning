...
$strpage         = get_string('modulename', 'finalsurvey');
$strpages        = get_string('modulenameplural', 'finalsurvey');
$strname         = get_string('name');
$strintro        = get_string('moduleintro');
$strlastmodified = get_string('lastmodified');

$PAGE->set_url('/mod/finalsurvey/index.php', array('id' => $course->id));
$PAGE->set_title($course->shortname.': '.$strpages);
$PAGE->set_heading($course->fullname);
$PAGE->navbar->add($strpages);
echo $OUTPUT->header();
echo $OUTPUT->heading($strpages);
if (!$pages = get_all_instances_in_course('finalsurvey', $course)) {
...
foreach ($pages as $page) {
    ...
    $table->data[] = array (
        ...
        format_module_intro('finalsurvey', $page, $cm->id));
}
