...
class mod_page_external extends external_api {
    ...
    public static function view_page($pageid) {
        ...
        $page = $DB->get_record('finalsurvey', array('id' => $params['pageid']), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($page, 'finalsurvey');
    ...
    public static function get_pages_by_courses($courseids = array()) {
        ...
        if (!empty($params['courseids'])) {
            ...
            $pages = get_all_instances_in_courses("finalsurvey", $courses);
            foreach ($pages as $page) {
                ...
                list($page->intro, $page->introformat) = external_format_text($page->intro,
                                                                $page->introformat, $context->id, 'mod_finalsurvey', 'intro', null);
                $page->introfiles = external_util::get_area_files($context->id, 'mod_finalsurvey', 'intro', false, false);

                $options = array('noclean' => true);
                list($page->content, $page->contentformat) = external_format_text($page->content, $page->contentformat,
                                                                $context->id, 'mod_finalsurvey', 'content', $page->revision, $options);
                $page->contentfiles = external_util::get_area_files($context->id, 'mod_finalsurvey', 'content');
