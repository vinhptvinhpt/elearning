...
if (!isguestuser()) {     // Guests can never edit their profile.
    ...
    if ($userid != 0 && $course_id != 0 && $module_id != 0) {
        ...
        $sql = "SELECT * from tms_learning_activity_logs WHERE user_id = $userid AND course_id = $course_id AND activity = $module_id ORDER BY id desc LIMIT 1";
        $log = $DB->get_records_sql($sql);

        $today = date('Y-m-d H:i:s', $now);

        $insert_query = "INSERT INTO `tms_learning_activity_logs`(user_id, course_id, activity, start_time, studying, url, created_at, updated_at) VALUES ($userid, $course_id, $module_id, $now, 1, '$url', '$today', '$today')";
...
//            $DB->execute($sql, $params);
            ...
            if ($type == 'init') { //create new log and change studying of other log to 0
                ...
                $DB->execute("UPDATE `tms_learning_activity_logs` SET end_time = $now, duration = $duration WHERE user_id = $userid AND course_id = $course_id AND activity = $module_id AND id = $last_id");
            }
        }
    }
}
