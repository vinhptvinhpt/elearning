        ...
        if (true === $enrolstatus) {
            ...
            if ($instance->id == $instanceid) {
                if ($data = $form->get_data()) {
                    ...
                    $noti_createdby = (int)$USER->id;
                    $noti_action = "create";
                    $noti = new stdClass();
                    $noti->type = $noti_type;
                    $noti->target = $noti_target;
                    $noti->status_send = $noti_status;
                    $noti->sendto = $noti_sendto;
                    $noti->createdby = $noti_createdby;
                    $noti->course_id = $noti_course_id;
                    $noti->action = $noti_action;
                    $content = json_encode($noti);
                    $noti_enroll = 'INSERT INTO tms_nofitications (type,target,status_send,course_id,sendto,createdby) values ("' . $noti->type . '","' . $noti->target . '", ' . $noti->status_send . ',' . $noti->course_id . ', '.$noti->sendto .','.$noti->createdby.')';
                    $DB->execute($noti_enroll);
                    $noti_quiz_log = 'INSERT INTO tms_nofitication_logs (type,target,action,content,status_send,course_id,sendto,createdby) values ("' . $noti->type . '","' . $noti->target . '","' . $noti->action . '",\'' . $content . '\', ' . $noti->status_send . ',' . $noti->course_id . ', '.$noti->sendto .','.$noti->createdby.')';
