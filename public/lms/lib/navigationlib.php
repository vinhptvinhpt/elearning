...
class navigation_node implements renderable {
    ...
    /** @var string Parameter to add sub menu collapse */
    public $submenu = null;
    ...
    public function __construct($properties) {
        if (is_array($properties)) {
            ...
            if (array_key_exists('submenu', $properties)) {
                $this->submenu = $properties['submenu'];
            }
...
class navigation_node_collection implements IteratorAggregate, Countable {
    ...
    // VinhPT
    // Add function get page
    /**
     * Return a page for this collection.
     *
     * @return string
     */
    ...
    // VinhPT
    // Add collection of navigation get page
    /**
     * Return a page for this collection.
     *
     * @return string
...
class global_navigation extends navigation_node {
    ...
    public function initialise() {
        ...
        // VinhPT
        // Hide my course from dashboard side bar
    ...
    public function add_course_essentials($coursenode, stdClass $course) {
        ...
        // VinhPT
        // [Customize] Add log of course to navbar
        $logcourse = $coursenode->add("Log", new moodle_url('/report/log/index.php?chooselog=1&id='.$course->id),
        self::TYPE_CONTAINER, get_string('log'), 'log', new pix_icon('i/badge', ''));
        ...
        if ($course->id > 1) {
            ...
            if ($coursenode && $coursenode->key != SITEID) {
                ...
                foreach ($coursenode->children as $child) {
                    // VinhPT
                    // [Customize] Not add children Badges, Competencies into navbar list
                    $listChildren = array("badgesview", "competencies");
                    if (!in_array($child->key, $listChildren)){
                        if ($child->action) {
...
class settings_navigation extends navigation_node {
        ...
        if (isloggedin() && !isguestuser() && (!isset($SESSION->load_navigation_admin) || $SESSION->load_navigation_admin)) {
            ...
            if (has_capability('moodle/site:configview', context_system::instance())) {
                ...
                if ($SESSION->load_navigation_admin && !$isadminpage) {
                    ...
                    // [VinhPT]
                    // Add back to lms node
                    // $siteadminnodetms = $this->add("Management", new moodle_url('/admin/search.php'),
                    //         self::TYPE_SITE_ADMIN, null, 'backtotms');
                    // $siteadminnodetms->id = 'expandable_branch_' . $siteadminnodetms->type . '_' .
                    //         clean_param($siteadminnodetms->key, PARAM_ALPHANUMEXT);
                    // $siteadminnodetms->requiresajaxloading = 'true';
                }
            }
        }
    ...
    protected function generate_user_settings($courseid, $userid, $gstitle='usercurrentsettings') {
        ...
        if ($gstitle == 'usercurrentsettings') {
            ...
            // This link doesn't have a unique display for course context so only display it under the user's profile.
            // VinhPT
            // Remove privates file from dashboard
            if ($issitecourse && $iscurrentuser && has_capability('moodle/user:manageownfiles', $usercontext)) {
                $url = new moodle_url('/user/files.php');
