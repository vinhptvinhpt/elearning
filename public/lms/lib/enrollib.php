...
// VinhPT
/**
 * Returns list of soft skill courses current $USER is enrolled in and can access
 *
 * The $fields param is a list of field names to ADD so name just the fields you really need,
 * which will be added and uniq'd.
 *
 ...
 * @return array
 */
function enrol_get_my_courses_softskill(
    $fields = null,
    $sort = null,
    $limit = 0,
    $courseids = [],
 ...
 * @return array
 */
function enrol_get_my_courses_offline(
    $fields = null,
    $sort = null,
    $limit = 0,
    $courseids = [],
 ...
 * @return array
 */
function enrol_get_my_courses_certificate(
    $fields = null,
    $sort = null,
    $limit = 0,
    $courseids = [],
    ...
    // Note: we can not use DISTINCT + text fields due to Oracle and MS limitations, that is why
    // we have the subselect there.
    $coursefields = $coursefields." ,c.is_end_quiz";
    ...
    //         c.*
    //     from
    //         mdl_user_enrolments as mu
    //     inner join mdl_user as u on
    //         u.id = mu.userid
    //     inner join mdl_enrol as e on
    //         e.id = mu.enrolid
    //     inner join mdl_course as c on
    //         c.id = e.courseid
    //     where
    //         c.category = 3 
    //         and u.id = ". $USER->id;
    ...
    foreach ($courses as $id => $course) {
        ...
        $courses[$id] = $course;
        array_push($course_visible, $id);
    }

    // [VinhPT][13.11.2019] Get all course except final exam course with category = 3
    $sql_check_final_exam = "
        select
            c.id as course_id,
            (select
            ...
            where
                ...
                and cm.course = c.id
                and cmc.userid = u.id) as user_course_learn,
            (select 
            ...
            where
                cs.section <> 0
                and cm.course = c.id) as total_module
        from
            mdl_user_enrolments as mu
        inner join mdl_user as u on
            u.id = mu.userid
    ...
    $condition_check = $DB->get_records_sql($sql_check_final_exam);
    $check_final_exam = 0;

    // Check if user_course_learn not equal total_module => not completed course
    foreach($condition_check as $check_ele){
        if(in_array($check_ele->course_id, $course_visible)){
            if ($check_ele->user_course_learn != $check_ele->total_module){
                $check_final_exam ++;
            }
            if($check_ele->total_module == 0 || $check_ele->user_course_learn == 0){
                $num_progress = 0;
            }else{
                $num_progress = round(($check_ele->user_course_learn / $check_ele->total_module) * 100, 2);
            }
            if($courses[$check_ele->course_id]){
                $courses[$check_ele->course_id]->progress = $num_progress;
            }else{
                continue;
            }
        }else{
    ...
    // Remove final exam course category = 3 if not meet condition
    // if ($check_final_exam != 0){
    //     foreach ($courses as $id => $course) {
    //         if ($course->is_end_quiz == 1){
    //             unset($condition_check[$id]);
    //             unset($courses[$id]);
    //             continue;
    //         }
    //     }
    ...
    // Sort course base on progress
    // Move 0% progress to end
    foreach($courses as $course){
        if ($course->progress == 0){
            $temp = $course;
            unset($courses[$course->id]);
            $courses[$temp->id] = $temp;
        }
    }

    // Move final quiz to end
    foreach ($courses as $course) {
        if ($course->is_end_quiz == 1) {
            $temp = $course;
            unset($courses[$course->id]);
