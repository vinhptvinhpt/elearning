...
function message_popup_render_navbar_output(\renderer_base $renderer) {
    ...
    if ($enabled) {
        ...
        $sql_count_unread_noti = "SELECT total_unread_msg FROM tms_user_detail WHERE user_id =".$USER->id;
        $unreadcount = (int) array_values($DB->get_records_sql($sql_count_unread_noti))[0]->total_unread_msg;
        $context = [
            'userid' => $USER->id,
