...
class api {
    ...
    public static function mark_all_notifications_as_read($touserid, $fromuserid = null) {
        ...
        $sql = "SELECT
                ...
                WHERE
                    TNL.sendto = ".$touserid;
        $notifications = array_values($DB->get_records_sql($sql));
        foreach ($notifications as $notification) {
            self::mark_notification_as_read($notification, $timeread = null, $notification->useridto);
        }
        // $notifications->close();
    }
     ...
     * @param int|null $timeread The time the message was marked as read, if null will default to time()
     */
    public static function mark_notification_as_read($notification, $timeread = null, $userid = 0) {
        global $DB;
        ...
        if (is_null($notification->timeread)) {
            ...
            $update_sql = "UPDATE tms_nofitication_logs SET is_read = ?, updated_at = ? WHERE id = ?";
            $DB->execute($update_sql, array(1, date('Y-m-d H:i:s'), $notification->id));
            if ($userid != 0){
                $update_sql_2 = "UPDATE tms_user_detail SET total_unread_msg = (total_unread_msg - 1) WHERE user_id = ? AND total_unread_msg > ?";
                $DB->execute($update_sql_2, array($userid, 0));
            }
