...
function set_coursemodule_visible($id, $visible, $visibleoncoursepage = 1)
{
    ...
    if (($cm->visible != $visible) && ($events = $DB->get_records('event', array('instance' => $cm->instance, 'modulename' => $modulename)))
    ) {
...
function move_section_to($course, $section, $destination, $ignorenumsections = false)
{
    ...
    if ((!$ignorenumsections && array_key_exists('numsections', $courseformatoptions) && ($destination > $courseformatoptions['numsections'])) || ($destination < 1)) {
        return false;
    }
...
class course_request
{
    ...
    public function get_category()
    {
        ...
        if (
            empty($this->properties->category) || !has_capability('moodle/course:changecategory', context_system::instance()) || (!$category = core_course_category::get($this->properties->category, IGNORE_MISSING, true))
        ) {
...
function course_get_user_navigation_options($context, $course = null)
{
    ...
    $options->blogs = !empty($CFG->enableblogs) && ($CFG->bloglevel == BLOG_GLOBAL_LEVEL || ($CFG->bloglevel == BLOG_SITE_LEVEL and ($isloggedin and !$isguestuser)))
...
function course_get_user_administration_options($course, $context)
{
    ...
    $options->editcompletion = $CFG->enablecompletion &&
        $course->enablecompletion && ($options->update || $hascompletiontabs);
...
function course_validate_dates($coursedata)
{
    ...
    if (
        !empty($coursedata['startdate']) && !empty($coursedata['enddate']) && ($coursedata['enddate'] < $coursedata['startdate'])
    ) {
 ...
 * @return Generator
 */
function course_get_enrolled_courses_softskill(
    int $limit = 0,
    int $offset = 0,
    string $sort = null,
    string $fields = null,
    ...
    while ($courses = enrol_get_my_courses_softskill($fields, $sort, $querylimit, $includecourses, false, $offset, $hiddencourses)) {
        yield from $courses;
 ...
 * @return Generator
 */
function course_get_enrolled_courses_certificate(
    int $limit = 0,
    int $offset = 0,
    string $sort = null,
    string $fields = null,
    ...
    while ($courses = enrol_get_my_courses_certificate($fields, $sort, $querylimit, $includecourses, false, $offset, $hiddencourses)) {
        yield from $courses;
 ...
 * @return Generator
 */
function course_get_enrolled_courses_offline(
    int $limit = 0,
    int $offset = 0,
    string $sort = null,
    string $fields = null,
    ...
    while ($courses = enrol_get_my_courses_offline($fields, $sort, $querylimit, $includecourses, false, $offset, $hiddencourses)) {
        yield from $courses;
...
function course_filter_courses_by_timeline_classification(
    ...
    foreach ($courses as $course) {
        ...
        if (($classification == COURSE_TIMELINE_HIDDEN && $pref) || (($classification == COURSE_TIMELINE_ALL || $classification == course_classify_for_timeline($course)) && !$pref)
        ) {
 ...
 * @return array
 */
function course_get_certificate_courses(int $userid = null, int $limit = 0, int $offset = 0, string $sort = null)
{
    ...
    $certificatecourses = $DB->get_records_sql($sql, $params, $offset, $limit);

    // Filter courses if last access field is hidden.
    $hiddenfields = array_flip(explode(',', $CFG->hiddenuserfields));

    if ($userid != $USER->id && isset($hiddenfields['lastaccess'])) {
        $certificatecourses = array_filter($certificatecourses, function ($course) {
            context_helper::preload_from_record($course);
            $context = context_course::instance($course->id, IGNORE_MISSING);
            // If last access was a hidden field, a user requesting info about another user would need permission to view hidden
            // fields.
