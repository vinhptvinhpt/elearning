...
class core_course_renderer extends plugin_renderer_base
{
     ...
     * @return string
     */
    public function course_info_box_course_view(stdClass $course)
    {
        $content = '';
        $content .= $this->output->box_start('generalbox info');
        $chelper = new coursecat_helper();
    ...
    public function get_imgurl($course)
    {
    ...
    protected function coursecat_coursebox(coursecat_helper $chelper, $course, $additionalclasses = '')
    {
        ...
        $coursenamelink = html_writer::link(
            ...
            $coursename,
            array('class' => $course->visible ? '' : 'dimmed')
        );
    ...
    protected function coursecat_coursebox_content(coursecat_helper $chelper, $course)
    {
        ...
        if ($img_url_DB){
            $contentimages .= html_writer::tag(
                'div',
                html_writer::empty_tag('img', array('src' => $root_url.$img_url_DB)),
                // [VinhPT] Add link to course image
                array(
                    'class' => 'courseimage',
                    'onclick' => 'window.location.href=\'/lms/course/view.php?id=' . $course->id . '\''
                )
            );
        }else{
            foreach ($course->get_course_overviewfiles() as $file) {
    ...
    protected function coursecat_category(coursecat_helper $chelper, $coursecat, $depth)
    {
        ...
        if ($chelper->get_subcat_depth() > 0 && $depth >= $chelper->get_subcat_depth()) {
            ...
            $classes[] = 'notloaded';
            if (
                $coursecat->get_children_count() || ($chelper->get_show_courses() >= self::COURSECAT_SHOW_COURSES_COLLAPSED && $coursecat->get_courses_count())
            ) {
    ...
    public function publicationselector($courseid)
    {
        ...
        $text .= html_writer::tag(
            'div',
            get_string('advertisepublication_help', 'hub'),
        ...
        $text .= html_writer::tag(
            'div',
            get_string('sharepublication_help', 'hub'),
            array('class' => 'publishhelp')
        );
    ...
    public function registeredonhublisting($courseid, $publications)
    {
        ...
        foreach ($publications as $publication) {
            ...
            if ($publication->timechecked > 1273127954) {
                ...
                $status .= $brtag . html_writer::tag(
                    'a',
                    get_string('updatestatus', 'hub'),
            ...
            // Add button cells.
            $cells = array(
                $publication->enrollable ?
                    get_string('advertised', 'hub') : get_string('shared', 'hub'),
                userdate(
                    $publication->timepublished,
    ...
    {
        $optionsyes = array(
            'sesskey' => sesskey(), 'id' => $publication->courseid,
        ...
        $publication->hubname = html_writer::tag(
            'a',
            'Moodle.net',
            array('href' => HUB_MOODLEORGHUBURL)
        );
        $formcontinue = new single_button(new moodle_url(
            "/course/publish/index.php",
            $optionsyes
        ), get_string('unpublish', 'hub'), 'post');
        $formcancel = new single_button(new moodle_url(
            "/course/publish/index.php",
            $optionsno
        ), get_string('cancel'), 'get');
        return $this->output->confirm(
            get_string('unpublishconfirmation', 'hub', $publication),
            $formcontinue,
            $formcancel
        );
    }
    ...
    public function sentbackupinfo($id)
    {
        ...
        $publishindexurl = new moodle_url(
            '/course/publish/index.php',
            array(
                'sesskey' => sesskey(), 'id' => $id,
                'published' => true
            )
        );
            ...
            switch ($v) {
                ...
                case FRONTPAGENEWS:
                    if ($SITE->newsitems) {
                        ...
                        require_once($CFG->dirroot . '/mod/forum/lib.php');
                        if (($newsforum = forum_get_course_forum($SITE->id, 'news')) && ($forumcontents = $this->frontpage_news($newsforum))
                        ) {
