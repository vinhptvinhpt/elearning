...
class core_course_external extends external_api {
    ...
    public static function get_enrolled_courses_by_timeline_classification_parameters() {
        return new external_function_parameters(
            array(
                ...
                'form' => new external_value(PARAM_TEXT, 'form', VALUE_DEFAULT, null)
            )
        );
    }
    ...
    public static function get_enrolled_courses_by_timeline_classification(
        ...
        if ($form == "softskill"){
            ...
            if ($classification == COURSE_TIMELINE_HIDDEN) {
                $courses = course_get_enrolled_courses_softskill(0, $offset, $sort, $fields,
        ...
        else if ($form == "offline"){
                    // If the timeline requires the hidden courses then restrict the result to only $hiddencourses else exclude.
            if ($classification == COURSE_TIMELINE_HIDDEN) {
                $courses = course_get_enrolled_courses_offline(0, $offset, $sort, $fields,
                    COURSE_DB_QUERY_LIMIT, $hiddencourses);
            } else {
                $courses = course_get_enrolled_courses_offline(0, $offset, $sort, $fields,
                    COURSE_DB_QUERY_LIMIT, [], $hiddencourses);
            ...
            // If the timeline requires the hidden courses then restrict the result to only $hiddencourses else exclude.
            if ($classification == COURSE_TIMELINE_HIDDEN) {
                $courses = course_get_enrolled_courses_certificate(0, $offset, $sort, $fields,
                    COURSE_DB_QUERY_LIMIT, $hiddencourses);
            } else {
                $courses = course_get_enrolled_courses_certificate(0, $offset, $sort, $fields,
                    COURSE_DB_QUERY_LIMIT, [], $hiddencourses);
     ...
     * @throws  invalid_parameter_exception
     */
    public static function get_activities_overview(
        string $classification,
        int $limit = 0,
        int $offset = 0,
        string $sort = null
     ...
     * @throws  invalid_parameter_exception
     */
    public static function get_enrolled_courses_offline(
        string $classification,
        int $limit = 0,
        int $offset = 0,
        string $sort = null
     ...
     * @throws  invalid_parameter_exception
     */
    public static function get_enrolled_courses_certificate(
        string $classification,
        int $limit = 0,
        int $offset = 0,
        string $sort = null
    ...
    // [VinhPT]
    // add function get_certificate_courses
    /**
     * Get last accessed courses adding additional course information like images.
     *
     * @param int $userid User id from which the courses will be obtained
     ...
     * @throws  invalid_parameter_exception
     */
    public static function get_certificate_courses(int $userid = 0, int $limit = 0, int $offset = 0, string $sort = null) {
        global $USER, $PAGE;
        ...
        // Reuse get_recent_courses_parameters
        $params = self::validate_parameters(self::get_recent_courses_parameters(),
            array(
                'userid' => $userid,
                'limit' => $limit,
        ...
        // course_get_certificate_courses
        $courses = course_get_certificate_courses($userid, $limit, $offset, $sort);
