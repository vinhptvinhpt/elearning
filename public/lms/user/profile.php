...
// [VinhPT] Get certificate for user
global $DB, $USER;
$sql_check_certificate = 'SELECT code FROM student_certificate WHERE status = 2 AND userid =' . $USER->id;
$check_certificate = $DB->get_records_sql($sql_check_certificate);
$img_certificate = 'No-Certificate';
if ($check_certificate) {
    $img_certificate = '/storage/upload/certificate/' . array_values($check_certificate)[0]->code . '.png';
}else{
    $img_certificate = '';    
}
...
<div id="popup_certificate" class="modal">
    ...
    <div class="modal-content">
        <span id="close_popup" class="close" onclick="closePopup()">&times;</span>
        <img id="cert_popup_img" src="<?= $img_certificate ?>" width="400" height="600" style="margin-left: auto; margin-right: auto;">
    </div>
</div>

<?php if ($img_certificate != '') { ?>
    <button id="certificate" type="button" class="btn btn-info py-2 px-2" style="font-size: 20px" onclick="openPopup()"><?= get_string('certificatetext') ?></button>
<?php
}
