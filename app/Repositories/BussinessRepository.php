...
//            }
    ...
    public function apiStore(Request $request)
    {
        try {
            ...
            if ($training_id > 0) {
                ...
                //\DB::beginTransaction();
                training_enrole($mdlUser->id, $category['category_id']);
                //\DB::commit();
            }
    ...
    public function apiGetCourseSampleTrainning(Request $request)
    {
        ...
        if ($this->keyword) {
            $lstData = $lstData->where(function ($query) {
                $query->orWhere('c.shortname', 'like', "%{$this->keyword}%")
                    ->orWhere('c.fullname', 'like', "%{$this->keyword}%");
            });
        }

        $lstData = $lstData->orderBy('id', 'desc')->groupBy('ttc.sample_id');
            ...
            if ($count_course > 0) {
                foreach ($lstCourseId as $course_id) {
                    ...
                    if ($data_trainning) {
                        ...
                        $context_cate = MdlContext::where('contextlevel', '=', \App\MdlUser::CONTEXT_COURSECAT)
                            ->where('instanceid', '=', $cate_id)->first();
    ...
    public function apiTrainningChange(Request $request)
    {
        try {
            ...
            if ($trainning['trainning_id'] != $trainning_id && $trainning_id != 0) {
                ...
                //enrole cho học viên theo khung năng lực
                //\DB::beginTransaction();
                training_enrole($user_id, $category['category_id']);
                //\DB::commit();
            }
