...
//        $select_array[] = 'course_final.timecompleted';
            ...
            if ($mode_select == 'completed_course') {
                $query->leftJoin('course_completion', function ($join) {
                    $join->on('mdl_user_enrolments.userid', '=', 'course_completion.userid');
                    $join->on('mdl_course.id', '=', 'course_completion.courseid');
                });
                $select_array[] = 'course_completion.timecompleted as completed';
            } elseif ($mode_select == 'learning_time') {
                $data_type = 'counter';
                $query->leftJoin('tms_learning_activity_logs', function ($join) {
                    $join->on('mdl_user_enrolments.userid', '=', 'tms_learning_activity_logs.user_id');
                    $join->on('mdl_course.id', '=', 'tms_learning_activity_logs.course_id');
                });
                $select_array[] = 'mdl_course.estimate_duration';
            ...
            if ($organization_id != 0) {
                $query->whereIn('tms_user_detail.user_id', $user_ids);
            }
            ...
            foreach ($list as $item) {
                ...
                if (strlen($item['user_id']) != 0) {
                    ...
                    if ($mode_select == 'learning_time') {
                        $user['duration'] = strlen($item['duration']) > 0 ? $item['duration'] : 0;
                        $user['estimate_duration'] = $item['estimate_duration'];
                        $user['category'] = $item['category'];
                    }
                    if ($data_type == 'person') { //certificated & complete training & complete course
    ...
    function finishQuery($query, $organization_id, $training_id, $course_id, $country, $start_date, $end_date, $mode_select, $in_training, $type_select = 'training_pqdl')
    {
        ...
        if (strlen($end_date) > 0) {
            ...
            if ($mode_select == 'learning_time') {
                $query = $query->where('tms_learning_activity_logs.updated_at', '<=', $end_date);
            }
        }
    }
            ...
            if (strlen($item['training_id']) != 0) {
                ...
                if (strlen($item['course_id']) != 0) {
                    ...
                    if (strlen($item['user_id']) != 0) {
                        ...
                        //hoàn thành, tính đến cấp course
    ...
    public static function buildDefaultReportObject(&$object, $object_id, $col0)
    {
    ...
    public static function pushUser(&$object, $key, $id, $user)
    {
        $object[$key][$id] = $user;
    }
...
//            return $e->getMessage();
    ...
    public function apiUserSchedule(Request $request)
    {
        ...
        //Lấy các khóa đã được enrol đang diễn ra
...
//            ->leftJoin('tms_traninning_programs as ttp', 'ttc.trainning_id', '=', 'ttp.id')
            ...
            ->select(
                ...
                'mc.shortname as shortname',
                'mc.startdate as startdate',
                'mc.enddate as enddate',
//                'mr.shortname as role_name',
        ...
        if ($status != 0) {
            switch ($status) {
                case 1:
                    $mdlUserEnr = $mdlUserEnr->where('mc.startdate', '<', strtotime($now));
                    $mdlUserEnr = $mdlUserEnr->where('mc.enddate', '>', strtotime($now));
                    break;
                case 2:
                    $mdlUserEnr = $mdlUserEnr->where('mc.startdate', '>', strtotime($now));
                    break;
        ...
        if ($this->keyword) {
            if ($this->keyword == 'offline' || $this->keyword == 'Offline') {
                $mdlUserEnr->whereIn('mc.category', [5]);
            } else if ($this->keyword == 'online' || $this->keyword == 'Online') {
                $mdlUserEnr->whereNotIn('mc.category', [2, 5]);
            } else {
                $mdlUserEnr = $mdlUserEnr->where(function ($q) {
                    $q->orWhere('mc.fullname', 'like', "%{$this->keyword}%")
...
//        $data = $data->orderBy('c.id', 'desc');//->distinct();;
        ...
        $sqlData = '(select c.id as course_id, c.shortname, c.fullname, ttp.name as trainning_name,
