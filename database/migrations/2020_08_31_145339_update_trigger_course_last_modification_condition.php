...
class UpdateTriggerCourseLastModificationCondition extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        DB::unprepared("CREATE TRIGGER course_last_update
            ...
            BEGIN
                ...
                AND NEW.action <> 'reviewed'
                AND NEW.action <> 'graded'
                AND NEW.edulevel = 1)
                THEN
                        UPDATE mdl_course
    ...
    public function down()
    {
        ...
        DB::unprepared("CREATE TRIGGER course_last_update
            AFTER INSERT ON `mdl_logstore_standard_log` FOR EACH ROW
            BEGIN
                IF (NEW.courseid <> 0 AND (NEW.action <> 'viewed' OR new.action <> 'reviewed') AND NEW.edulevel = 1)
                THEN
                        UPDATE mdl_course
                        SET last_modify_user = NEW.userid, last_modify_time = NEW.timecreated, last_modify_action = CONCAT(NEW.action, ' ', NEW.target)
                        WHERE id = NEW.courseid;
