            ...
            <div class="col">
                <nav class="breadcrumb" aria-label="breadcrumb">
                    <ol class="breadcrumb bg-transparent px-0">
                        ...
                        <li class="breadcrumb-item">
                            <router-link v-if="type === 'teacher'"
                                         :to="{ name: 'EditUserById', params: { user_id: user_id }, query: { type: type } }">
            ...
            <div class="row mx-0">
                <div class="col-12 hk-sec-wrapper">
                    <h5 class="hk-sec-title" v-if="type === 'system'">
                        {{trans.get('keys.chinh_sua_thong_tin_nguoi_dung')}}</h5>
                    <h5 class="hk-sec-title" v-else-if="type === 'teacher'">
                        {{trans.get('keys.chinh_sua_thong_tin_giang_vien')}}</h5>
                        ...
                        <div class="col-12 col-lg-3">
                            <div class="card">
                                <div style="padding: 10px;position: relative;" v-if="users.avatar" class="text-center">
                                    <a class="remove_avatar" @click="removeAvatar()"
                                       :title="trans.get('keys.go_avatar')">
                                ...
                                <div class="form-group">
                                    <div class="card-body">
                                        <a class="mb-10 btnShowChangePass"
                                           style="cursor: pointer;display:block;text-decoration: underline;"
                                           @click="showChangePass()">{{trans.get('keys.thay_doi_mat_khau')}} > </a>

                                        <div class="showChangePass" style="display:none;">
                                        <span class="wrap_password pass">
                                            <input autocomplete="false" v-model="password" type="password"
                                                   id="inputPassword" :placeholder="trans.get('keys.mat_khau') + ' *'"
                                            ...
                                            <label class="text-danger message error passStyleError"
                                                   style="display: none;">{{trans.get('keys.mat_khau_sai_dinh_dang')}}</label>
                                            <span class="wrap_password pass_conf">
                                            <input autocomplete="false" v-model="passwordConf" type="password"
                                                   id="inputPasswordConfirm"
                                                   :placeholder="trans.get('keys.nhap_lai_mat_khau') + ' *'"
                                                   class="form-control mb-4" @input="validate_password">
                                            <i class="fa fa-check-circle-o label_validate" aria-hidden="true"></i>
                                        </span>
                                            <label class="text-danger message error passConfError"
                                                   style="display: none;">{{trans.get('keys.vui_long_nhap_mat_khau_moi')}}</label>
                                            <label class="text-danger message error passConfStyleError"
                                                   style="display: none;">{{trans.get('keys.mat_khau_sai_dinh_dang')}}</label>
                                            <label style="display: block;">
                                                <input type="checkbox" @click="viewPassword()">{{trans.get('keys.hien_thi_mat_khau')}}
                                            </label>
                                            <button type="button" class="btn btn-primary btn-sm"
                                                    @click="updatePassword()">{{trans.get('keys.cap_nhat_mat_khau')}}
                                            </button>
                                            <label class="text-danger message error passwordError"
                                                   style="display: none;">{{trans.get('keys.mat_khau_khong_khop_nhau')}}</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="col-12 col-lg-9">
                            <form action="" class="form-row">
                                ...
                                <div class="col-md-4 col-sm-6 form-group">
                                    ...
                                    <input type="text" id="inputUsername" v-model="users.username"
                                           class="form-control mb-4" autocomplete="false" disabled
                                           @input="changeRequired('inputUsername')" readonly>
                                ...
                                <div class="col-md-4 col-sm-6 form-group">
                                    ...
                                    <input type="text" id="inputFullname" v-model="users.fullname"
                                           class="form-control mb-4" @input="changeRequired('inputFullname')">
                                ...
                                <div class="col-md-4 col-sm-6 form-group">
                                    ...
                                    <input type="text" id="inputAddress" v-model="users.address"
                                           class="form-control mb-4">
                                </div>
                                <div class="col-md-4 col-sm-6 form-group">
                                    <label for="inputCity">{{trans.get('keys.van_phong')}} *</label>
                                    <input v-model="users.city" type="text" id="inputCity"
                                           :placeholder="trans.get('keys.nhap_van_phong')" class="form-control mb-4">
...
<!--                                        <option v-for="role in roles" :value="role.id">{{ trans.has('keys.' + role.name) ? trans.get('keys.' + role.name) : role.name.charAt(0).toUpperCase() + role.name.slice(1) }}</option>-->
                                ...
                                <div class="col-md-4 col-sm-6 form-group">
                                    ...
                                    <input v-model="users.start_time" type="date" id="inputTimeStart"
                                           class="form-control mb-4">
                                </div>

                                <div class="col-md-4 col-sm-6 form-group" v-if="role_type !== 'market'">
                                    <label for="inputWorkingStatus">{{trans.get('keys.tinh_trang_lam_viec')}}</label>
                                    <select id="inputWorkingStatus" class="form-control custom-select"
                                            v-model="users.working_status">
                                ...
                                <div v-if="type === 'system'" class="col-md-4 col-sm-6 form-group">
                                    ...
                                    <label v-if="!users.employee.organization_id"
                                           class="text-danger organization_required hide">{{trans.get('keys.truong_bat_buoc_phai_nhap')}}</label>
                            ...
                            <div class="button-list" v-if="users.deleted === 0">
                                <button type="button" class="btn btn-primary btn-sm" @click="updateUser()">
                                    {{trans.get('keys.cap_nhat_thong_tin')}}
                                </button>
                                ...
                                <button style="float:right;"
                                        @click.prevent="deletePost('/system/user/delete/'+users.user_id)"
                                        class="btn btn-sm btn-danger">
                                    {{trans.get('keys.xoa')}}
                                </button>
                            </div>
                            <div class="text-right" v-else>
                                <a :title="trans.get('keys.khoi_phuc_tai_khoan')" href="#"
                                   class="btn btn-sm btn-success" @click="restoreUser(users.user_id)">
