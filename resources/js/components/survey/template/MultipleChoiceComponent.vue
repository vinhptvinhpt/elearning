<template>
    <div class="row">
        ...
        <div class="col-12">
            <h6>{{trans.get('keys.cau_hoi')}} {{index_question + 1}}:  <label v-html="question.question_data[0].content"></label></h6>
        </div>
        <div class="col-12" v-for="(ans,index) in question.question_data[0].answers" style="padding-left:30px;">
            <div class="custom-control custom-radio custom-control-inline mb-2 radio-primary">
                <input type="radio" :id="'customRadio-'+ans.id" :name="'customRadio-'+question.id+index_question"
                       @click="getAnswerChoose(question.question_data[0].id,question.question_data[0].type_question,ans.id,ans.content)"
                       :value="ans.id"
