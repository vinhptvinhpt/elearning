      ...
      <div class="col">
        <nav class="breadcrumb" aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent px-0">
            ...
            <li class="breadcrumb-item"><router-link to="/tms/question/list">{{ trans.get('keys.danh_sach_cau_hoi') }}</router-link></li>
          ...
          <div class="card">
            <div class="card-header d-flex justify-content-between">
              <a role="button" data-toggle="collapse" href="#collapse_1"
                 aria-expanded="true"><i class="fal fa-plus mr-3"></i>{{trans.get('keys.sua_cau_hoi')}}</a>
            </div>
                  ...
                  <div class="col-12 col-lg-12">
                    <form action="" class="form-row">
                      <div class="col-4 form-group">
                        ...
                        <select v-model="question.survey_id" class="form-control" id="survey_id">
                      ...
                      <div class="col-4 form-group">
                        <label for="type_question">{{trans.get('keys.chon_loai_cau_hoi')}} *</label>
                        <select v-model="question.type_question" class="form-control"
                                @change="onChange()"
                                id="type_question">
                          <option value="">{{trans.get('keys.chon_loai_cau_hoi')}}</option>
                          <option value="multiplechoice">{{trans.get('keys.lua_chon_dap_an')}}</option>
                      ...
                      <div class="col-12 form-group">
                        <label for="article_ckeditor">{{trans.get('keys.noi_dung_cau_hoi')}} (*)</label>
                        <textarea v-model="question.content" class="form-control" rows="5"
                                  id="article_ckeditor"
                    ...
                    <div id="multiple-answer">
                      ...
                      <div class="button-list" style="text-align: center;">
                        <button @click="addAnswer()" type="button" class="btn-sm btn-success">{{trans.get('keys.them_dap_an')}}</button>
                    ...
                    <div id="group-question">
                      ...
                      <div class="button-list" style="text-align: center;">
                        <button @click="addQuestionChild()" type="button"
                                class="btn-sm btn-success">{{trans.get('keys.them_cau_hoi_con')}}</button>
                      </div>
                    </div>
                    <div class="button-list">
                      <button @click="updateQuestion()" type="button" class="btn btn-primary">{{trans.get('keys.sua')}}
                      </button>
    ...
    methods: {
      ...
      updateQuestion() {
        ...
        if (this.question.type_question === 'multiplechoice' && this.anwsers.length === 0) {
          swal({
            title: 'Thông báo',
            text: 'Bạn chưa nhập câu trả lời',
            type: "error",
        ...
        if (this.question.type_question === 'group' && (this.question_childs.length === 0 || this.anwsers.length === 0)) {
          swal({
            title: 'Thông báo',
            text: 'Bạn chưa nhập câu trả lời hoặc câu hỏi con',
            type: "error",
            showCancelButton: false,
            closeOnConfirm: false,
            showLoaderOnConfirm: true
