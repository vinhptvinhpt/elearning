                    ...
                    <div class="col-lg-5 col-md-5 col-sm-12 mb-3">
                        <label>
                            <h6 class="hk-sec-title">
                                {{trans.get('keys.danh_sach_khoa_hoc_kha_dung')}}</h6>
                        </label>
                        <div class="row">
                            <div class="col-12">
                                ...
                                <b-tab :title="trans.get('keys.thu_tu')">
                                    ...
                                    <template v-if="list.length !== 0">
                                        <div class="row text-center action-order">
                                            <a @click="sort" title="Reset Order"
                                               class="btn btn-sm btn-icon btn-danger btn-icon-style-2"
                                               style="color: rgb(255, 255, 255);"><span class="btn-icon-wrap"><i
                                                    class="fal fa-refresh"></i></span></a>
                                            <a @click="saveOrder" title="Save Order"
                                        ...
                                        <div class="row">
                                            <draggable
                                                ...
                                                <transition-group type="transition" :name="!drag ? 'flip-list' : null">
                                                    <li class="list-group-item" v-for="(element, index)  in list"
                                                        :key="element.order_no">
                                                        #{{index + 1}} | <b>{{ element.shortname }}</b> |
                                                        {{element.fullname}}
                                                    </li>
                                                </transition-group>
    ...
    export default {
        ...
        methods: {
            getListSampleCourse(paged) {
                axios.post('/api/trainning/api_training_course', {
            ...
            enrolUserToCourse() {
                ...
                loader.fadeIn();
                axios.post('/api/trainning/add_course_to_training', {
            ...
            removeEnrolUserToCourse() {
                ...
                let loader = $('.preloader-it');
                loader.fadeIn();
                axios.post('/api/trainning/remove_course_from_training', {
                ...
                if (this.allSelected) {
                    ...
                    if (countEnrol > 0) {
                        for (var i = 0; i < countEnrol; i++) {
                            this.userEnrols.push(this.sampleCourses[i]);
                        }
                    }
                }
            },
