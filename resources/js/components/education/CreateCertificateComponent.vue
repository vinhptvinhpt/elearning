        ...
        <div class="card-body">
          <p>
            <input type="file" ref="file" name="file" class="dropify" accept="image/*"
                   @change="selectedFile"/>
            <label
              class="required text-danger image_required hide">{{trans.get('keys.truong_bat_buoc_phai_nhap')}}</label>
          </p>
        </div>
      </div>
    </div>
      ...
      <form action="" class="form-row hk-sec-wrapper">
        ...
        <div class="col-12">
          <div class="button-list">
            <button @click="createCertificate()" type="button" class="btn btn-primary btn-sm">
              {{trans.get('keys.tao')}}
            </button>
